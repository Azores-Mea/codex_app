...
public class SyntaxErrorActivity extends AppCompatActivity {
    ...
    private Map<Integer, ExerciseActivity.Submission> submissionCache = new HashMap<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_syntax_error);
    ...
    private void loadExercises(String lessonId) {
        DatabaseReference ref = FirebaseDatabase.getInstance()
                .getReference("assessment/" + lessonId + "/FindingSyntaxError");
    ...
    private void loadPreviousAnswers(String lessonId) {
        ...
        userAnswersRef.get().addOnSuccessListener(snapshot -> {
                    ...
                    try {
                        ...
                        if (savedCode != null && savedOutput != null && isCorrect != null) {
                            submissionCache.put(exerciseIndex,
                                    new ExerciseActivity.Submission(savedCode, savedOutput, isCorrect));
                        }
                    } catch (NumberFormatException e) {
                        Log.e("SyntaxErrorActivity", "Invalid exercise index: " + exerciseSnap.getKey());
                    }
                }
            ...
            Log.e("SyntaxErrorActivity", "Failed to load previous answers: " + e.getMessage());
        });
    }
    ...
    private void loadCurrentExercise() {
        ...
        if (exercise != null) {
            ...
            if (submissionCache.containsKey(currentIndex)) {
                ExerciseActivity.Submission prev = submissionCache.get(currentIndex);
                if (prev != null) {
    ...
    private void runCode() {
        ...
        api.execute(req).enqueue(new Callback<PistonExecuteResponse>() {
            ...
            public void onResponse(Call<PistonExecuteResponse> call,
                ...
                submissionCache.put(currentIndex, new ExerciseActivity.Submission(userCode, actual, isCorrect));
    ...
    private void saveUserAnswer(int exerciseIndex, String code, String output, boolean isCorrect) {
        ...
        userAnswersRef.setValue(answerData)
                .addOnSuccessListener(aVoid -> {
                    Log.d("SyntaxErrorActivity", "Answer saved successfully for exercise " + exerciseIndex);
                })
                .addOnFailureListener(e -> {
                    Log.e("SyntaxErrorActivity", "Failed to save answer: " + e.getMessage());
                });
    }
    ...
    private void submitExercises() {
        correctExercises = 0;
        for (Map.Entry<Integer, ExerciseActivity.Submission> entry : submissionCache.entrySet()) {
            ExerciseActivity.Submission submission = entry.getValue();
        ...
        DatabaseReference syntaxResultsRef = FirebaseDatabase.getInstance()
                .getReference("syntaxErrorResults")
                .child(String.valueOf(sessionManager.getUserId()))
                .child(lessonId);
        ...
        syntaxResultsRef.setValue(resultData)
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(this,
                    ...
                    resultIntent.putExtra("syntaxErrorCompleted", allCorrect);
